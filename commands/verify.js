    ...
    async execute(interaction) {
        ...
        .then(collected => {
            ...
            if (reaction.emoji.name === '✅') {
                message.react('✅')
                message.reply('Zatwierdzono wniosek!')
                .then(msg => {
                    setTimeout(() => msg.delete(), 5000)
                    interaction.client.users.cache.get(id).send('Wniosek został zatwierdzony! Możesz teraz korzystać z kanałów serwera');
                    })
            } else {
                message.react('❌')
                message.reply('Odrzucono wniosek!')
                .then(msg => {
                    setTimeout(() => msg.delete(), 5000)
                    interaction.client.users.cache.get(id).send('Wniosek został odrzucony! Jeśli uważasz, że jest to błąd, skontaktuj się ze starostą');
                    })
            }
        });
        })
        .catch(collected => {
            message.reply(':x: Brak reakcji na wniosek w ciągu 60 sekund. Automatyzacja wyłączona.');
        });
